...
class Args:
    ...
    match_special_tokens_to: str | None = None
    normalizer_strategy: str = "replace" # keep, replace, remove
    pretokenizer_strategy: str = "replace" # keep, replace
...
def byteify(
    tokenizer,
    normalizer_strategy="keep",
    pretokenizer_strategy="keep",
    ...
    if len(fill_bytes) > 0:
        logger.warning(f"{len(fill_bytes)} bytes not in surface forms.")
    ...
    if make_whitespace_consistent:
        ...
        for i in range(len(surface_forms)):
            if surface_forms[i] in allowed_whitespace_tokens:
                ...
                sum(c in extra_whitespace for c in surface_forms[i]) > 1
                or len(surface_forms[i].strip()) == 0
            ):
    ...
    new_normalizer_data = {"type": "Prepend", "prepend": " "}
        ...
        "pretokenizers": [
            {
                ...
                "pattern": {
                    "Regex": (
                        split_regex if split_regex is not None else DEFAULT_SPLIT_REGEX
                    )
                },
                "behavior": "Removed",
                "invert": True,
    ...
    if normalizer_strategy == "replace":
    ...
    if (
        ...
        fix_postprocessor_data(
            match_special_tokens_to_data["post_processor"],
            {surface_form: i for i, surface_form in enumerate(surface_forms)},
        )
...
if __name__ == "__main__":
    ...
    source_tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_name)

    if args.match_special_tokens_to is not None:
        match_special_tokens_to = AutoTokenizer.from_pretrained(args.match_special_tokens_to)
    else:
        match_special_tokens_to = None

    target_tokenizer, _ = byteify(
        source_tokenizer,
        normalizer_strategy=args.normalizer_strategy,
        pretokenizer_strategy=args.pretokenizer_strategy,
        split_regex=args.split_regex,
        match_special_tokens_to=match_special_tokens_to
    )
